...
func NewRouter(dbPool *pgxpool.Pool) http.Handler {
	...
	mux.HandleFunc("GET /api/v1/documents/{id}/collaborators", middleware.RequireAuth(documentHandler.GetCollaborators))
	mux.HandleFunc("DELETE /api/v1/documents/{id}/share/{collaboratorId}", middleware.RequireAuth(documentHandler.RemoveCollaborator))
